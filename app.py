...
# 1. 페이지 설정 및 브랜드 테마
st.set_page_config(
    page_title="LH | Rebar Digital Twin QC", 
    layout="wide",
...
if selected_tab == "철근 시공오차 분석":
    ...
    if os.path.exists(csv_file):
        ...
        with m5:
            st.markdown('<div style="font-size:0.85rem; padding:10px; border:2px solid #009944; border-radius:10px; background:white;"><b>품질 기준:</b> PASS < 20mm | ERROR > 30mm</div>', unsafe_allow_html=True)
...
# ------------------------------------------------------------------
    ...
    with r_col:
        ...
        st.markdown("---")
        st.info("💡 **정합 모델**은 설계 좌표계에 맞춰 보정된 최종 결과물입니다.")
