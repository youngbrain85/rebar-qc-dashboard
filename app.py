...
# 로고 파일을 Base64로 인코딩하는 함수 (이미지 경로 문제 해결)
...
# 로고 HTML 생성 (깃허브의 lh_logo.png 사용)
logo_html = ""
...
st.markdown(f"""
    ...
    div[data-testid="stStatusWidget"] {{display: none !important;}} /* Manage app 버튼 숨기기 */
    ...
    /* 배경 디자인 (연두색 포인트 블록) - z-index를 음수로 설정하여 가려짐 방지 */
    .stApp::before {{
        content: '';
        position: fixed;
        top: 0;
    ...
    div[data-testid="stMetricLabel"] {{ font-size: 1.1rem !important; color: #475569 !important; font-weight: 700 !important; }}
    div[data-testid="stMetricValue"] {{ font-size: 3rem !important; font-weight: 900 !important; color: #0f172a !important; }}

    /* 테이블(DataFrame) 스타일 (가려짐 방지 배경 설정) */
    .stDataFrame {{
...
if selected_tab == "철근 시공오차 분석":
    ...
    if os.path.exists(csv_file):
        ...
        with left_col:
            if os.path.exists(glb_file):
                ...
                <model-viewer src="data:model/gltf-binary;base64,{b64_glb}" 
                              ...
                              camera-controls touch-action="pan-y" shadow-intensity="1" orientation="-90deg -90deg -90deg" exposure="1.2" environment-image="neutral">
                </model-viewer>
                """
        ...
        with right_col:
            ...
            fig_bar = px.bar(bar_data, x='Count', y='상태', orientation='h', color='Status',
                             color_discrete_map={'PASS': '#94a3b8', 'CAUTION': '#009944', 'ERROR': '#f59e0b', 'MISSING': '#ef4444'},
                             text='Count')
            
            fig_bar.update_layout(showlegend=False, height=230, margin=dict(l=0, r=20, t=10, b=10),
                                 paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                                 font_color="#0f172a", xaxis_title="", yaxis_title="",
                                 xaxis=dict(showgrid=True, gridcolor='#f1f5f9'), font=dict(size=16, weight='bold'))
        ...
        st.error("분석 결과 데이터(CSV)가 없습니다.")

else:
    # 기타 페이지 플레이스홀더
    st.markdown(f"""
        <div class="placeholder-content">
            <h1 style="font-size: 3rem; font-weight: 900; margin-bottom: 20px;">{selected_tab}</h1>
            <p style="font-size: 1.2rem;">해당 기능은 현재 구현 준비 중입니다.</p>
            <div style="margin-top: 30px; font-size: 5rem; opacity: 0.2;">🏗️</div>
        </div>
    """, unsafe_allow_html=True)
