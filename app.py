        ...
        try:
            ...
            if isinstance(scene, trimesh.Scene):
                for name, geom in scene.geometry.items():
                    # [색상 추출 로직 강화]
                    target_color = '#808080' # 기본값
                    
                    # 재질 이름(mat_PASS 등)에서 상태를 추출하여 색상 매핑
                    found_status = False
                    if hasattr(geom.visual, 'material'):
                        mat_name = getattr(geom.visual.material, 'name', '').upper()
                    ...
                    # 만약 이름에서 못 찾았다면 diffuse 색상 확인
                    if not found_status and hasattr(geom.visual, 'material'):
                        if hasattr(geom.visual.material, 'diffuse'):
                            c = geom.visual.material.diffuse
                            target_color = f'rgb({c[0]},{c[1]},{c[2]})'
